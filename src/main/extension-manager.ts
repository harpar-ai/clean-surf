...
export class ExtensionManager {
  ...
  async installCrx(crxPath: string): Promise<string> {
    // Validate path came from file dialog (must be absolute, must be .crx)
    if (!path.isAbsolute(crxPath) || !crxPath.endsWith('.crx')) {
    ...
    const rawName = path.basename(crxPath, '.crx')
    const name = rawName.replace(/[^a-zA-Z0-9_-]/g, '_').slice(0, 64)
    const destDir = path.join(extensionsDir, name)
    // Prevent path traversal: ensure destDir is inside extensionsDir
    if (!destDir.startsWith(extensionsDir + path.sep)) {
    ...
    // Use execFileSync with array args (no shell) to prevent injection
    const { execFileSync } = await import('child_process')
    execFileSync('unzip', ['-o', tmpZip, '-d', destDir])
    fs.unlinkSync(tmpZip)

    await this.session.loadExtension(destDir)
    return destDir
  }
