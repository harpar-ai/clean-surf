      ...
      ${items.map(item => `
        <div class="history-item" data-id="${item.id}">
          ...
          <div class="item-content">
            <a class="item-title" href="${item.url}" title="${item.url}">${item.title}</a>
            <div class="item-url">${item.url}</div>
          </div>
          <button class="delete-btn" onclick="deleteItem('${item.id}')" title="Remove from history">✕</button>
        </div>
      `).join('')}
    </div>
  `).join('')
...
<div class="header">
  ...
  <form class="search-box" action="cleanshell://history" method="get">
    ...
    <input type="text" name="q" placeholder="Search history" value="${query}" autofocus>
  </form>
